...
func NewServer(host string,name string) *Server {
    BindHost=host;
...
func (s *Server) regWsdlBindPort(name string) {
	...
	// binding
	soapio := wsdl.SoapBodyIO{SoapBody: wsdl.SoapBody{Use: "encoded",EncodingStyle:"http://schemas.xmlsoap.org/soap/encoding/"}}
...
func (s *Server) request(w http.ResponseWriter, de *xml.Decoder, startEle *xml.StartElement) {
	...
	out := v.Elem().FieldByName("Out").Interface()
	ret :=reflect.ValueOf(out)
	returnInfo:=ret.FieldByName("ReturnInfo").Interface().(string)
	//returns := v.Elem().FieldByName("Out").Interface()
	buf := bytes.NewBuffer([]byte{})
	buf.Write([]byte(returnInfo))
